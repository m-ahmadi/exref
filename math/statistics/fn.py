...
def frac_diff_expanding_window(x=[], d=0, thres=0.01):
	N = len(x)
	...
	for i in range(skip, N):
		r.append( np.dot(w[-(i+1):], x[:i+1]) )
	
	return r

def frac_diff_fixed_window(x=[], d=0, thres=0.00001):
	N = len(x)
	...
	for i in range(width, N):
		r.append( np.dot(W, x[i-width:i+1]) )
