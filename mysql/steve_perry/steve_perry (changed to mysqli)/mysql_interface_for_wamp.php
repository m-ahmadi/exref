...
function doSQLStatement($a_sql_string, $select_statement_flag)
{
	...
	if ($result) {
	   // if ($myrowcount == 0 && !$errors_found)
		...
		$myMsg .= "<br><font color=red>MySQL No: ".mysqli_errno($db);
		$myMsg .= "<br>MySQL Error: ".mysqli_error($db);
		$myMsg .= "<br>SQL: ".$b_sql_string;
		$myMsg .= "<br>MySQL Affected Rows: ".mysqli_affected_rows($db)."</font><br>";
...
function writeSQLHistory($a_sql_string)
{
	...
	$sqlhistory_filename = $DOCUMENT_ROOT.'/steve_perry/sql_history.txt';
	...
	if (!empty($result))
	{
		if ($select_statement_flag == "true" || $show_statement_flag == "true")
		{
			...
			for ($k = 0; $k < $numfields; $k++)
			{
				 ...
				 $selectResults .= mysqli_fetch_field($result)->name;
